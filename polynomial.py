...
class Polynomial(Ring):
    ...
    def binomial_theorem(self, m: Ring.Element, a: Ring.Element, exp: int) -> Polynomial.Element:
        ...
        Разложение (mx+a)^exp
        """
        logger.debug('Polynomial.binomial_theorem. self=%s, m=%s, a=%s, exp=%s.', self, m, a, exp)
        poly = self()
        for k in range(exp + 1):
            coeff = comb(exp, k) * (m ** (exp - k)) * (a ** k)
            poly[exp - k] = coeff
            logger.debug('Polynomial.binomial_theorem. self=%s, m=%s, a=%s, exp=%s, poly=%s',
                         self, m, a, exp, poly)
        return poly
    ...
    class Element(Ring.Element):
        ...
        def __init__(
            ...
            logger.debug('Polynomial.Element.__init__. ring=%s, raw_poly=%s', self.ring, raw_poly)
            if not raw_poly:
                self.value = dict()
        ...
        def __mul__(self, other: 'Polynomial.Element' | Ring.Element) -> 'Polynomial.Element':
            ...
            logger.debug('Polynomial.__mul__. self=%s, other=%s, result=%s.', self, other, result)
            return result
        ...
        def __fermat_little_theorem(self) -> 'Polynomial.Element':
            ...
            for exp,coeff in self.monomials():
                while exp >= p:
                    exp = exp // p + exp % p
                poly[exp] += coeff
            logger.debug('Polynomial.Element.fermat_little_theorem. self=%s, poly=%s.', self, poly)
            return poly
        ...
        def derivative(self) -> 'Polynomial.Element':
            ...
            logger.debug('Polynomial.Element.derivative. self=%s, derivative=%s.', self, derivative)
            return derivative
        ...
        def roots(self):
            ...
            if isinstance(self.ring.basic_ring, IntegerModRing):
                ...
                if len(primary_results) == 1:
                    logger.debug('Polynomial.Element.roots. self=%s, module=%s, primary_results=%s.'
